...
// Extend Express Request type to include apiKeyAuth flag
declare global {
  namespace Express {
    interface Request {
      apiKeyAuth?: boolean;
...
function _getAuditUserId(req: Request): string | null {
...
router.put('/profile', upload.fields([
  ...
  // For API key auth, user.id starts with "apikey:" - use null for audit
  const auditUserId = req.user?.id?.startsWith('apikey:') ? null : (req.user?.id || 'system');
...
router.put('/settings', asyncHandler(async (req: Request, res: Response) => {
  ...
  await DatabaseService.getInstance().logAudit(
    restaurantId,
    req.user?.id?.startsWith('apikey:') ? null : (req.user?.id || 'system'),
