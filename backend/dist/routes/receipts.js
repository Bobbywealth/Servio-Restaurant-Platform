 ...
 */
router.post('/request-upload', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    await DatabaseService_1.DatabaseService.getInstance().logAudit(userId, 'request_receipt_upload', 'receipt', uploadToken, { description, expiresAt: expiresAt.toISOString() });
    logger_1.logger.info(`Receipt upload link requested by user ${userId}`);
 ...
 */
router.get('/upload-status/:token', (0, errorHandler_1.asyncHandler)(async (req, res) => {
 ...
 */
router.post('/process/:token', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { token } = req.params;
    ...
    await DatabaseService_1.DatabaseService.getInstance().logAudit(userId || 'system', 'process_receipt', 'receipt', token, { fileName, fileSize, extractedItemCount: mockProcessingResult.extractedData.items.length });
    logger_1.logger.info(`Receipt processed: ${fileName} (${fileSize} bytes)`);
    res.json({
        success: true,
 ...
 */
router.get('/:id/details', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
