...
class VoiceOrderingService {
    ...
    async createOrder(input) {
        ...
        let phone = typeof input?.customer?.phone === 'string' && input.customer.phone.trim().length > 0
        ...
        if (!phone && input?.callId) {
            try {
                const row = await db.get('SELECT from_phone FROM call_logs WHERE call_id = ? ORDER BY created_at DESC LIMIT 1', [input.callId]);
                if (row?.from_phone)
