...
const getUnreadCount = async (restaurantId, role, userId) => {
...
router.post('/:id/read', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const db = DatabaseService_1.DatabaseService.getInstance().getDatabase();
    await db.run(`INSERT INTO notification_reads (notification_id, user_id, read_at)
     VALUES (?, ?, CURRENT_TIMESTAMP)
     ON CONFLICT (notification_id, user_id) DO UPDATE SET read_at = excluded.read_at`, [id, userId]);
    const unreadCount = await getUnreadCount(restaurantId, role, userId);
 ...
 */
router.post('/read-all', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const user = req.user;
    const restaurantId = user.restaurantId;
    const userId = user.id;
    const role = user.role;
