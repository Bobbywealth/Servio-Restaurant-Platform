...
const ensureUploadsDir = async (subdir = '') => {
    const uploadsPath = path_1.default.join(process.cwd(), 'uploads', 'restaurants', subdir);
    try {
...
router.put('/profile', upload.fields([
    ...
    if (req.files) {
        ...
        if (files.logo && files.logo[0]) {
            ...
            const logoPath = path_1.default.join(uploadsPath, logoFileName);
        ...
        if (files.coverImage && files.coverImage[0]) {
            ...
            const coverPath = path_1.default.join(uploadsPath, coverFileName);
            await (0, sharp_1.default)(coverFile.buffer)
...
router.post('/links', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const existingLink = await db.get('SELECT id FROM restaurant_links WHERE restaurant_id = ? AND url_path = ?', [restaurantId, urlPath]);
    if (existingLink) {
        return res.status(400).json({
            success: false,
    ...
    const qrFilePath = path_1.default.join(qrCodePath, qrFileName);
    await qrcode_1.default.toFile(qrFilePath, fullUrl, {
        width: 300,
        margin: 2,
        color: {
            dark: '#000000',
...
router.put('/links/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (urlPath !== undefined) {
        ...
        if (urlPath !== existingLink.url_path) {
            const conflictingLink = await db.get('SELECT id FROM restaurant_links WHERE restaurant_id = ? AND url_path = ? AND id != ?', [existingLink.restaurant_id, urlPath, id]);
            if (conflictingLink) {
                return res.status(400).json({
                    success: false,
                    error: { message: 'URL path already exists' }
...
router.delete('/links/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (link.qr_code_url) {
        const qrFilePath = path_1.default.join(process.cwd(), 'uploads', 'restaurants', 'qr-codes', path_1.default.basename(link.qr_code_url));
        try {
            await promises_1.default.unlink(qrFilePath);
        }
        catch (error) {
...
router.post('/links/:id/click', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    await db.run('UPDATE restaurant_links SET click_count = click_count + 1 WHERE id = ?', [id]);
