 ...
 * Clock in a user
 */
router.post('/clock-in', (0, errorHandler_1.asyncHandler)(async (req, res) => {
 ...
 * Clock out a user
 */
router.post('/clock-out', (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `, [clockOutTime, totalHours.toFixed(2), notes || null, timeEntry.id]);
    ...
    await bus_1.eventBus.emit('staff.clock_out', {
        restaurantId: user.restaurant_id,
    ...
    logger_1.logger.info(`User ${user.name} clocked out at ${clockOutTime}, worked ${totalHours.toFixed(2)} hours`);
    res.json({
        success: true,
        data: {
 ...
 */
router.post('/start-break', (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `, [breakId, timeEntry.id, breakStart]);
    ...
    await bus_1.eventBus.emit('staff.break_start', {
 ...
 */
router.post('/end-break', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { userId } = req.body;
    if (!userId) {
        return res.status(400).json({
            success: false,
  ...
  `, [totalBreaks.total_break_minutes, timeEntry.id]);
    ...
    const staff = await db.get('SELECT name FROM users WHERE id = ?', [userId]);
    await bus_1.eventBus.emit('staff.break_end', {
        restaurantId: timeEntry.restaurant_id,
 ...
 */
router.get('/current-staff', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const db = DatabaseService_1.DatabaseService.getInstance().getDatabase();
    const dialect = DatabaseService_1.DatabaseService.getInstance().getDialect();
    const currentStaff = dialect === 'postgres'
        ? await db.all(`
          SELECT
            u.id as user_id,
            u.name,
 ...
 */
router.get('/entries', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { userId, startDate, endDate, status = 'all', // 'active', 'completed', 'all'
...
router.get('/stats', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], endDate = new Date().toISOString().split('T')[0], userId } = req.query;
    const db = DatabaseService_1.DatabaseService.getInstance().getDatabase();
    const dialect = DatabaseService_1.DatabaseService.getInstance().getDialect();
    let baseQuery = `
    FROM time_entries te
